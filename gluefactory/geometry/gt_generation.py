...
def gt_patch_matches_from_homography(data, H, patch_size=16, pos_th=3, neg_th=0, **kw):
    ...
    idx_placeholder = m_count0.new_tensor(1)
    m0 = torch.where(negative.any(-1), unmatched, ignore)  # First, find unmatched, undecidable points
    m1 = torch.where(negative.any(-2), unmatched, ignore)
    m0 = torch.where(positive.any(-1), idx_placeholder, m0)  # Then, mark positive points
    m1 = torch.where(positive.any(-2), idx_placeholder, m1)
